...
def get_project_data(project_id: str) -> dict:
    ...
    return {
        ...
        "hook_candidates": load_json("hook_candidates.json"),
        ...
        "story_plans": load_json("story_plans.json"),
        "edit_plan": load_json("edit_plan.json"),
        "retention_audit": load_json("retention_audit.json"),
...
def stage_hook_candidates(project_id: str) -> dict:
    """从 shots 中提取 hook 候选，独立评分"""
    date_dir = os.path.join(ROOT_DIR, project_id)
    ...
    update_progress(project_id, "hook_candidates", 10, "评估 hook 候选...")
    ...
    for shot in shots:
        ...
        ps = shot.get("platform_scores", {})
        q = shot.get("quality", {})
        types = shot.get("shot_types", [])
        summary = shot.get("visual_summary", "")
        actions = shot.get("actions", [])

        # hook 评分维度
        hook_base = ps.get("hook", 5) * 10  # 0-100

        # 静音可懂: 全景/成果展示/整理后 类型更容易静音看懂
        works_muted = any(t in types for t in ["全景", "成果展示", "整理后", "特写"])
        if works_muted:
            hook_base += 5

        # 画面变化: 对比类 +5, 动作类 +3
        if any(t in types for t in ["整理后", "成果展示"]):
            hook_base += 8
        elif any(t in types for t in ["动作过程", "整理中"]):
            hook_base += 3

        # 问题感: summary 含疑问词或反差暗示
        question_words = ["为什么", "怎么", "居然", "终于", "原来", "没想到"]
        has_question = any(w in summary for w in question_words)
        if has_question:
            hook_base += 5

        # 1秒知主题: 整理后/成果展示 类型最容易一眼看懂
        clarity_bonus = ps.get("clarity", 5) * 2
        hook_base += clarity_bonus - 10  # normalize

        # 封面适合度
        cover_bonus = ps.get("cover_value", 5) * 2
        hook_base += cover_bonus - 10

        score = max(0, min(100, hook_base))

        # 生成视觉问题
        visual_question = ""
        if "整理后" in types or "成果展示" in types:
            visual_question = "这个变化是怎么发生的？"
        elif "动作过程" in types:
            visual_question = f"这是在做什么？"
        elif "全景" in types:
            visual_question = "这个地方经历了什么？"
        elif "特写" in types:
            visual_question = "这个细节意味着什么？"
        ...
        if "整理后" in types or "成果展示" in types:
            ...
            hook_type = "细节特写"
        elif "全景" in types and ps.get("beauty", 0) >= 7:
            hook_type = "美感空镜"
    ...
    atomic_write_json(os.path.join(date_dir, "hook_candidates.json"), result)

    update_progress(project_id, "hook_candidates", 100,
                   f"Hook 评分完成: {len(candidates)} 个候选, 最高 {candidates[0]['score'] if candidates else 0} 分")
    return result
...
STORY_PLANS_PROMPT = """你是一位短视频编剧，擅长竖屏生活记录短片。

## 任务
根据以下素材信息，生成 3 个不同角度的剪辑方案。每个方案是一个独立的视频构思。
...
## 要求
1. 每个方案时长 {target_duration} 秒左右，9:16 竖屏
2. 3 个方案必须是不同角度，例如：
   - 方案 A: 整理前后对比（突出变化）
   - 方案 B: 花园日记（轻松日常记录）
   - 方案 C: 治愈氛围（重美感和情绪）
3. 每个方案的开头必须从 hook 候选中选，或从素材中选最有冲击力的片段
4. 每个片段要说明"观众此刻会产生什么问题"（viewer_question）
5. 结尾要有互动引导
6. **source 字段必须使用实际视频文件名**（如 5894_raw.MP4）
7. 为每个方案给出推荐指数 (0-100)
...
{{
  "plans": [
    {{
      "plan_id": "plan_A",
      "angle": "整理前后对比",
      "hook_clip": "5902_raw.MP4 00:03-00:06 整理后成果",
      "reason": "前后反差明显，适合提高开头停留",
      "title": "视频标题",
      "cover_text": "封面文字(6-8字)",
      "tone": "整体调性",
...
def stage_story_script(project_id: str, theme: str = "日常生活记录", target_duration: int = 60) -> dict:
    ...
    # 读取 shots
    shots = _load_shots(date_dir)

    # 读取 hook 候选
    hook_data = load_json(os.path.join(date_dir, "hook_candidates.json"))
    hook_info = "(无 hook 数据)"
    if hook_data and hook_data.get("candidates"):
        top_hooks = hook_data["candidates"][:5]
        hook_info = "\n".join(
            f"- #{h['rank']} [{h['shot_id']}] {h['source']} {h['start']:.1f}-{h['end']:.1f}s "
            f"类型:{h['hook_type']} 分数:{h['score']} "
            f"问题:{h.get('visual_question','')}"
            for h in top_hooks
        )

    # 读取转写结果
    transcriptions = load_json(os.path.join(date_dir, "transcription.json"), [])

    # 构建素材描述
    materials = _build_materials_description(shots)
    ...
    update_progress(project_id, "story_script", 30, "调用 AI 生成 3 个候选方案...")
    ...
    prompt = STORY_PLANS_PROMPT.format(
        theme=theme,
        hook_info=hook_info,
    ...
    try:
        ...
        parsed = extract_json_from_text(text)
        if parsed and isinstance(parsed, dict) and "plans" in parsed:
            plans = parsed["plans"]
        elif parsed and isinstance(parsed, list):
            plans = parsed
    except Exception as e:
        print(f"  故事方案 API 错误: {e}")

    # Fallback: 无 API 或解析失败时生成默认方案
    if not plans:
        plans = _fallback_story_plans(project_id, theme, shots, target_duration)

    # 保存 story_plans.json
    plans_data = {"plans": plans, "selected_plan": ""}
    atomic_write_json(os.path.join(date_dir, "story_plans.json"), plans_data)

    # 向后兼容：同时保存 story_script.json（取第一个方案）
    if plans:
        script = _plan_to_script(plans[0], target_duration)
        atomic_write_json(os.path.join(date_dir, "story_script.json"), script)

    update_progress(project_id, "story_script", 100,
                   f"生成 {len(plans)} 个候选方案: {', '.join(p.get('angle','?') for p in plans)}")
    return {"plans": plans, "count": len(plans)}


def _load_shots(date_dir: str) -> list[dict]:
    """加载 shots 或 analysis 数据"""
    shots_path = os.path.join(date_dir, "shots.json")
    if os.path.exists(shots_path):
        with open(shots_path, encoding="utf-8") as f:
...
def _build_materials_description(shots: list[dict]) -> str:
    """构建按推荐用途分组的素材描述"""
    materials = ""
...
def _fallback_story_plans(project_id: str, theme: str, shots: list[dict], target_duration: int) -> list[dict]:
    """无 API 时的 fallback: 生成 3 个默认方案"""
    raw_dir = os.path.join(ROOT_DIR, project_id, "raw")
    videos = []
    if os.path.isdir(raw_dir):
        videos = sorted([f for f in os.listdir(raw_dir) if os.path.splitext(f)[1].lower() in VIDEO_EXTS])

    non_deleted = [s for s in shots if not s.get("delete")]

    def make_plan(plan_id, angle, title, score):
        clips = []
        for i, shot in enumerate(non_deleted[:6]):
            clips.append({
                "shot_id": shot.get("shot_id", ""),
                "role": STORY_SLOTS[i % len(STORY_SLOTS)]["role"] if i < len(STORY_SLOTS) else "action",
                "source": shot.get("source", ""),
                "start": float(shot.get("start", 0)),
                "end": float(shot.get("end", 10)),
                "duration": float(shot.get("duration", 10)),
                "caption": shot.get("visual_summary", ""),
                "viewer_question": "",
                "voiceover_text": "",
            })
        return {
            "plan_id": plan_id,
            "angle": angle,
            "hook_clip": f"{clips[0]['source']} {clips[0]['start']:.1f}-{clips[0]['end']:.1f}s" if clips else "",
            "reason": f"基于素材自动推荐",
            "title": title,
            "cover_text": title[:8],
            "tone": "清新自然",
    ...
    return [
        make_plan("plan_A", f"{theme}前后对比", f"今天的{theme}", 80),
        make_plan("plan_B", f"{theme}日记", f"{theme}小记", 65),
        make_plan("plan_C", f"治愈{theme}", f"安静的{theme}", 50),
    ]


def _plan_to_script(plan: dict, target_duration: int) -> dict:
    """将单个方案转换为旧版 story_script 格式"""
    clips = []
    for c in plan.get("clips", []):
        clips.append({
            "id": c.get("shot_id", ""),
            "role": c.get("role", "action"),
            "source": c.get("source", ""),
            "start": c.get("start", 0),
            "end": c.get("end", 0),
            "duration": c.get("duration", 0),
            "caption": c.get("caption", ""),
            "voiceover_text": c.get("voiceover_text", ""),
            "transition": "cut",
            "note": c.get("viewer_question", ""),
        })
    return {
        "title": plan.get("title", ""),
        "subtitle": plan.get("angle", ""),
        "storyline": plan.get("reason", ""),
        "tone": plan.get("tone", ""),
...
def stage_retention_check(project_id: str) -> dict:
    """检查剪辑方案的节奏和留存潜力"""
    date_dir = os.path.join(ROOT_DIR, project_id)

    # 读取 edit_plan
    plan = load_json(os.path.join(date_dir, "edit_plan.json"))
    if not plan or not plan.get("clips"):
        return {"pass": False, "issues": ["无剪辑计划"]}
    ...
    # 检查 1: 前 3 秒是否有钩子
    first_3s_clips = [c for c in clips if c.get("timeline_start", 0) < 3]
    has_hook = False
    for c in first_3s_clips:
        role = c.get("role", "")
        if role in ("opening", "result") or "hook" in c.get("platform_goal", ""):
            has_hook = True
            break
        ps_hook = 0  # 不在 clip 里直接有 platform_scores
        if c.get("edit_style") == "fade":
            has_hook = True  # fade 开场也算
    if not has_hook:
        issues.append("前 3 秒没有明确钩子，建议用整理后成果或动作引入开头")
        suggestions.append("从 hook_candidates 中选择高分片段替换开头")

    # 检查 2: 前 10 秒是否交代主题
    first_10s_roles = [c.get("role", "") for c in clips if c.get("timeline_end", 0) <= 10]
    has_theme = any(r in first_10s_roles for r in ("opening", "space", "action_intro"))
    if not has_theme:
        issues.append("前 10 秒未交代主题，观众可能不知道视频在讲什么")
        suggestions.append("在前 10 秒加入环境交代或动作引入片段")

    # 检查 3: 连续同类画面是否超过 8 秒
    prev_role = None
    same_role_start = 0
    same_role_duration = 0
    max_same_duration = 0
    for c in clips:
        role = c.get("role", "")
        dur = c.get("duration", 0)
        if role == prev_role:
            same_role_duration += dur
        else:
            same_role_duration = dur
            same_role_start = c.get("timeline_start", 0)
        prev_role = role
        max_same_duration = max(max_same_duration, same_role_duration)

    if max_same_duration > 8:
        issues.append(f"连续同类画面最长 {max_same_duration:.0f} 秒，建议压缩到 8 秒以内")
        suggestions.append("同类画面之间插入其他角色片段作节奏过渡")

    # 检查 4: 是否有前后对比
    roles = [c.get("role", "") for c in clips]
    has_before_after = "action" in roles and "result" in roles
    if not has_before_after:
        issues.append("缺少前后对比，短视频的对比感能提高完播率")
        suggestions.append("确保包含动作过程(action)和成果展示(result)")

    # 检查 5: 是否有记忆点
    memory_keywords = ["红桶", "第一刀", "满地", "成果", "全景", "特写", "对比"]
    has_memory = False
    for c in clips:
        caption = c.get("caption", "") + c.get("note", "")
        if any(k in caption for k in memory_keywords):
            has_memory = True
            break
    if not has_memory and len(clips) >= 4:
        issues.append("缺少记忆点，建议加入红桶/第一刀/成果全景等标志性画面")
        suggestions.append("从 shots 中选择 visual_summary 含标志性元素的片段")

    # 检查 6: 结尾是否有互动
    last_clips = clips[-2:] if len(clips) >= 2 else clips
    has_interaction = False
    for c in last_clips:
        if c.get("role") == "ending" or "互动" in c.get("note", ""):
            has_interaction = True
    if not has_interaction:
        issues.append("结尾缺少互动引导，建议加评论问题")
        suggestions.append("在结尾字幕或旁白中加入引导评论的问题")

    # 检查 7: 总时长
    total_duration = plan.get("actual_duration", 0)
    if total_duration > 90:
        issues.append(f"总时长 {total_duration:.0f} 秒偏长，建议控制在 60-75 秒")
    elif total_duration < 30:
        issues.append(f"总时长 {total_duration:.0f} 秒偏短，建议至少 45 秒")

    audit = {
        "plan_id": plan.get("version", ""),
        "pass": len(issues) == 0,
        "issues": issues,
        "suggestions": suggestions,
        "hooks_in_first_3s": has_hook,
        "theme_in_first_10s": has_theme,
        "has_before_after": has_before_after,
        "has_memory_point": has_memory,
        "has_ending_interaction": has_interaction,
        "max_same_type_duration": max_same_duration,
        "total_duration": total_duration,
    }

    atomic_write_json(os.path.join(date_dir, "retention_audit.json"), audit)

    status = "✅ 通过" if audit["pass"] else f"⚠ {len(issues)} 个问题"
    update_progress(project_id, "retention_check", 100, f"节奏检查: {status}")
    return audit
...
- platform_variants: 每个平台独立的标题和封面字，风格不同
  - douyin: 标题口语化、有悬念，封面字有力
  - xiaohongshu: 标题略文艺，封面字干净
  - shipinhao: 标题偏生活感，节奏可以慢一点
...
def stage_publish_pack(project_id: str, theme: str = "日常生活记录") -> dict:
    ...
    if not pack:
        ...
        pack = {
            ...
            "platform_variants": {
                "douyin": {"title": title, "cover_text": title[:6]},
                "xiaohongshu": {"title": f"普通人{theme}记录", "cover_text": theme[:4]},
                "shipinhao": {"title": f"{theme}的日常", "cover_text": theme[:4]},
            },
        }
...
def run_full_pipeline(project_id: str, theme: str = "日常生活记录", target_duration: int = 60) -> dict:
    ...
    try:
        # Stage 2: 关键帧
        update_progress(project_id, "pipeline", 8, "抽取关键帧...")
        results["stages"]["keyframes"] = stage_keyframes(project_id)

        # Stage 3: Contact sheets + 时间索引
        update_progress(project_id, "pipeline", 16, "生成 Contact Sheet...")
        results["stages"]["contact_sheets"] = stage_contact_sheets(project_id)

        # Stage 4: 片段级视觉分析
        update_progress(project_id, "pipeline", 25, "片段级视觉分析...")
        results["stages"]["visual_analysis"] = stage_visual_analysis(project_id, theme)

        # Stage 4.5: Hook 候选评分
        update_progress(project_id, "pipeline", 33, "Hook 评分...")
        results["stages"]["hook_candidates"] = stage_hook_candidates(project_id)

        # Stage 5: 转写
        update_progress(project_id, "pipeline", 40, "音频转字幕...")
        results["stages"]["transcription"] = stage_transcription(project_id)

        # Stage 5.5: 模板选择
        update_progress(project_id, "pipeline", 45, "选择视频模板...")
        results["stages"]["template_select"] = stage_select_template(project_id)

        # Stage 6: 生成 3 个候选方案
        update_progress(project_id, "pipeline", 55, "生成 3 个候选方案...")
        results["stages"]["story_script"] = stage_story_script(project_id, theme, target_duration=target_duration)

        # Stage 7: 编辑计划 (剪辑决策书)
        update_progress(project_id, "pipeline", 65, "生成剪辑决策书...")
        results["stages"]["edit_plan"] = stage_edit_plan(project_id, theme)

        # Stage 8.5: 节奏检查
        update_progress(project_id, "pipeline", 72, "节奏检查...")
        results["stages"]["retention_check"] = stage_retention_check(project_id)

        # Stage 9: 发布包
        update_progress(project_id, "pipeline", 80, "生成发布包...")
        results["stages"]["publish_pack"] = stage_publish_pack(project_id, theme)

        # Stage 8: 草稿渲染 (不渲染正式版，只出粗剪供审片)
        update_progress(project_id, "pipeline", 90, "草稿渲染...")
        results["stages"]["render"] = stage_render(project_id, mode="draft")

        update_progress(project_id, "pipeline", 100, "✅ 全流程完成! 请审片后确认方案再渲染正式版")
    except Exception as e:
...
@app.route("/api/projects/<path:project_id>/hook-candidates", methods=["POST"])
def api_hook_candidates(project_id):
    def run():
        try:
            return stage_hook_candidates(project_id)
        except Exception as e:
            return {"error": str(e)}
    return jsonify(run())


@app.route("/api/projects/<path:project_id>/retention-check", methods=["POST"])
def api_retention_check(project_id):
    def run():
        try:
            return stage_retention_check(project_id)
        except Exception as e:
            return {"error": str(e)}
    return jsonify(run())
